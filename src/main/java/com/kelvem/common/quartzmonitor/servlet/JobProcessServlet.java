package com.kelvem.common.quartzmonitor.servlet;
...
import com.kelvem.common.quartzmonitor.service.ISchedulerService;
import com.kelvem.common.utils.WebUtil;

/**
 * Servlet implementation class JobProcessServlet
 * 
 * http://kelvem-pc2:8080/sshsample/JobProcessServlet?jobtype=100&action=query
 */
public class JobProcessServlet extends HttpServlet {
	
	private static Log log = LogFactory.getLog(JobProcessServlet.class);

	private static final long serialVersionUID = -8814767269190444071L;
	...
	public void init(ServletConfig config) throws ServletException {
		schedulerService = (ISchedulerService)WebUtil.getBean("schedulerService");
	}
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		log.info("JobProcessServlet recieve command (action=" + action + ", jobtype=" + jobtype + ")");
	...
	private void addSimpleTrigger(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// 获取界面以p_参数
		Map<String, Object> map = WebUtils.getParametersStartingWith(request, "p_");
		Map<String, String> filterMap = new HashMap<String, String>();
		for (String key : map.keySet()) {
			filterMap.put(key, ((String[])map.get(key))[0]);
		}
	...
	private void addCronTriggerBy(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			request.getRequestDispatcher("/quartz/list.jsp").forward(request, response);
